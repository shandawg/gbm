                     ...
                     ...)
{
   ...
   if (x$distribution$name=="multinomial")
   {
   ...
   else if ((x$distribution$name=="asymmetric") && (type=="response")){
       X$y <- exp(y)
   }
      ...
      warning("type 'response' only implemented for 'bernoulli', 'poisson', 'gamma', 'tweedie', 'asymmetric','multinomial', and 'pairwise'. Ignoring" )
   }
   else { X$y <- y }
